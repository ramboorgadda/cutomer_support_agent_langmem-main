...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(
        ...
        extra="ignore",)
    app_name: str = "AI Copilot for Support Agent"
    ...
    dashboard_api_url: str = "http://localhost:8080"
    
    
    def resolve(self, path: str) -> Path:
        return Path(path) if Path(path).is_absolute() else self.workspace_dir / path
    @property
...
def ensure_directories(settings: Settings | None = None) -> None:
    ...
    settings.chroma_mem0_path.mkdir(parents=True, exist_ok=True)
