...
class CustomerMemoryStore(InMemoryStore):
    def __init__(self, settings: Settings, llm: Any):
        super().__init__()
        self._settings = settings
        self._store = self._build_store()
        _ = llm  # Currently unused, but may be needed for future extensions
        self._manage_memory_tool = create_manage_memory_tool(namespace = ("memories","{memory_user_id}"),
                                                            store=self._store,
                                                            actions_permitted=("create",),)
    
    def _build_store(self) -> InMemoryStore:
        if not self._settings.google_api_key:
            logger.info("Google API key not found. Memory store will not have embedding capabilities.")
            return InMemoryStore()
        model_name = self._settings.effective_google_embedding_model
        try:
            embeddings = init_embeddings(model_name=model_name, api_key=self._settings.google_api_key)
